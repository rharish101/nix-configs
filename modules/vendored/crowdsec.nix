      ...
      type = lib.types.submodule {
        options = {
          ...
          branch = lib.mkOption {
            type = lib.types.str;
            default = "master";
            description = "The git branch on which cscli is going to fetch configurations. See `https://docs.crowdsec.net/docs/configuration/crowdsec_configuration/#hub_branch` for more information";
            example = [
              "master"
              "v1.4.3"
              "v1.4.2"
      ...
      systemd.services = {
        ...
        crowdsec = {
          ...
          serviceConfig = {
            ...
            Group = cfg.group;
            Type = "simple";
            RestartSec = 5; # 60; # The Local API server crashes due to temporary networking issues; the log processor crashes because it wants us to reload the config.
            LimitNOFILE = 65536;
            NoNewPrivileges = true;
            LockPersonality = true;
            RemoveIPC = true;
      ...
      networking.firewall.allowedTCPPorts =
        let
          parsePortFromURLOption =
            url: option:
            builtins.addErrorContext "`${option}` requires a port to be specified, but we failed to parse a port from '${url}'" (
